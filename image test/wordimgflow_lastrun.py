...
def setupWindow(expInfo=None, win=None):
    ...
    if expInfo is not None:
        ...
        if win._monitorFrameRate is None:
            win._monitorFrameRate = win.getActualFrameRate(infoMsg='Measuring...............frame.................rate')
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    instruct = visual.TextStim(win=win, name='instruct',
        text='',
        font='Noto sans',
        units='norm', pos=(-0.5, 0.7), draggable=False, height=0.07, wrapWidth=None, ori=0.0, 
        color='white', colorSpace='rgb', opacity=None, 
        languageStyle='LTR',
        depth=0.0);
    ...
    image = visual.ImageStim(
        ...
        ori=0.0, pos=(-0.5, 0), draggable=False, size=(1, None),
    ...
    imageword = visual.ImageStim(
        win=win,
        name='imageword', units='norm', 
        image='default.png', mask=None, anchor='center',
    ...
    for thisTrial in trials:
        ...
        # create an object to store info about Routine wordshow
        wordshow = data.Routine(
            name='wordshow',
            components=[image, instructt, plsuer],
        )
        wordshow.status = NOT_STARTED
        continueRoutine = True
        # update component parameters for each repeat
        image.setImage(worddir)
        instructt.setText('image == word? press `z` if yes, `x` if no')
        # store start times for wordshow
        wordshow.tStartRefresh = win.getFutureFlipTime(clock=globalClock)
        wordshow.tStart = globalClock.getTime(format='float')
        wordshow.status = STARTED
        thisExp.addData('wordshow.started', wordshow.tStart)
        wordshow.maxDuration = None
        # keep track of which components have finished
        wordshowComponents = wordshow.components
        for thisComponent in wordshow.components:
            thisComponent.tStart = None
            thisComponent.tStop = None
            thisComponent.tStartRefresh = None
            thisComponent.tStopRefresh = None
        ...
        if isinstance(trials, data.TrialHandler2) and thisTrial.thisN != trials.thisTrial.thisN:
            continueRoutine = False
        wordshow.forceEnded = routineForceEnded = not continueRoutine
        while continueRoutine and routineTimer.getTime() < 0.5:
            ...
            # check if all components have finished
            if not continueRoutine:  # a component has requested a forced-end of Routine
                wordshow.forceEnded = routineForceEnded = True
                break
            continueRoutine = False  # will revert to True if at least one component still running
            for thisComponent in wordshow.components:
                if hasattr(thisComponent, "status") and thisComponent.status != FINISHED:
        ...
        for thisComponent in wordshow.components:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        # store stop times for wordshow
        wordshow.tStop = globalClock.getTime(format='float')
        wordshow.tStopRefresh = tThisFlipGlobal
        thisExp.addData('wordshow.stopped', wordshow.tStop)
        # using non-slip timing so subtract the expected duration of this Routine (unless ended on request)
        if wordshow.maxDurationReached:
            routineTimer.addTime(-wordshow.maxDuration)
        elif wordshow.forceEnded:
            routineTimer.reset()
        else:
        ...
        letterselect = data.Routine(
            name='letterselect',
            components=[imageword, instructor, plus, key_resp],
        )
        ...
        while continueRoutine:
            ...
            # if imageword is starting this frame...
            if imageword.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                # keep track of start time/frame for later
                imageword.frameNStart = frameN  # exact frame index
            ...
            # if imageword is active this frame...
            if imageword.status == STARTED:
                # update params
                pass
